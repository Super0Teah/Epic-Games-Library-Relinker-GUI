...
class LibraryHandler:
    def _create_entry(self, m: dict, loc: str, status: str, can_import: bool = False) -> dict:
        app_name = ""
        try:
        ...
        return {
            ...
            "status":       status,
            "app_name":     app_name if status == "Linked" else "",
            ...
            try:
                ...
                if os.path.exists(dat_path):
                    with open(dat_path, "r", encoding="utf-8") as f:
                        dat = json.load(f)
                        for entry in dat.get("InstallationList", []):
            ...
            if games_path and os.path.exists(games_path):
                ...
                for game in gdm._game_data_list:
                    ...
                    if game_loc not in linked_paths:
                        can_import = ManifestCapture.read_mancpn(game.game_folder.path) is not None
                        results_games.append({
